...
各队的分数、以及每个游戏当前的进度（问答已用过的题目、是非题答到第几
题、时间轴摆放的顺序、转盘的自定义选项）都会自动保存在浏览器本地存储
中，刷新页面或关闭浏览器后再打开也不会丢失。

每个页面顶部都有统一的导航菜单，可以在各个游戏之间快速切换。点击导航
栏右侧的"⋮ 更多"，可以：

- **导出存档** — 把队伍分数和所有游戏进度打包下载成一个 JSON 文件，
  方便备份或换到另一台电脑继续使用。
- **导入存档** — 选择之前导出的存档文件，一键恢复队伍和进度。
- **重置本局进度** — 只清空当前这个游戏的进度，不影响分数和其他游戏。
- **重置全部数据** — 清空所有队伍分数和全部游戏进度，重新开始。

`scoreboard.html`（队伍计分板）页面底部也有同样的"导出/导入存档"按钮，
方便在课前课后集中管理存档。
